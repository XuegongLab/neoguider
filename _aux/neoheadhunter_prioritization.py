...
def compute_immunogenic_probs(data, paramset, passflag):
    ...
    rescued_by_bindstab_ind = (indicator(data.BindStab > paramset.resue_by_bindstab_thres) & data.BindLevel.isin(['SB', 'WB']))
    ...
    presented_not_recog_vals = sorted(ad for (ad, ind) in zip(data.RNA_normAD, presented_not_recog) if ind and not math.isnan(ad))
    presented_and_recog_vals = sorted(ad for (ad, ind) in zip(data.RNA_normAD, presented_and_recog) if ind and not math.isnan(ad))
    ...
    # bindlevel_penal = 0 # indicator(~data.BindLevel.isin(['SB'])) * 3 * (1 - rescued_by_bindstab_ind * indicator(med_immuno_strength >= 0))
    # rnaqlevel_penal = indicator(t2rna_nfilters > 0)           * 3 * (1 - rescued_by_bindstab_ind * indicator(med_immuno_strength >= 0))
    rnaqlevel_penal = 0
    bindlevel_penal = data.BindLevel.isin(['WB']) * 3 + data.BindLevel.isin(['NB']) * 6
    ...
    log_odds_ratio = (t1BindAff / (t1BindAff + data.MT_BindAff)
            ...
            # tier-2 filters 
            - (
                4 * indicator(data.ET_BindAff >= t2BindAff) +
                3 * indicator(data.BindStab <= t2BindStab) +
                5 * indicator(data.Quantification <= t2Abundance)
            )
...
def datarank(data, outcsv, paramset, drop_cols = [], passflag = 0x0):
    ...
    if outcsv:
        data.to_csv(outcsv, header=1, sep='\t', index=0, float_format='%.10f', na_rep = 'NA')
...
If the keyword rerank is in function,
    ...
    if not isna(args.truth_file):
        ...
        combdata.to_csv(args.output_file + '.validation' , header=1, sep='\t', index=0, float_format='%10g', na_rep = 'NA')
