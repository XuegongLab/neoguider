# This file is copied from python3.10/site-packages/scipy/stats/_mannwhitneyu.py and then modified
# to support the computation of p values for the null hypothesis that the effect size is greater than a threshold
...
MannwhitneyuResult = namedtuple('MannwhitneyuResult', ('statistic', 'pvalue', 'z', 'Udiff', 'Udiff_sd'))
...
def mannwhitneyu(x, y, use_continuity=True, alternative="two-sided",
    ...
    *p*-value produced using the normal approximation.
    ...
    z = Udiff = Udiff_sd = np.nan
    if method == "exact":
        p = _mwu_state.sf(U.astype(int), n1, n2)
    elif method == "asymptotic":
        z, Udiff, Udiff_sd = _get_mwu_z(U, n1, n2, ranks, continuity=use_continuity)
        p = stats.norm.sf(z)
    p *= f
    ...
    return MannwhitneyuResult(U1, p, z, Udiff, Udiff_sd)
