...
parser.add_argument('-s', '--seed',   help=F'NumPy random-number generator seed', type=int, default=8)
...
x0e = np.array([MAX_VAL+0.20]) - np.random.random(8)*0.2
x0f = np.array([MAX_VAL+0.40]) - np.random.random(8)*0.2
x0g = np.array([MAX_VAL+0.60]) - np.random.random(8)*0.2
x0h = np.array([MAX_VAL+0.80]) - np.random.random(8)*0.2
x0i = np.array([MAX_VAL+1.00]) - np.random.random(8)*0.2
...
x1e = np.array([MAX_VAL+0.5]) - np.random.random(100)**0.33*0.5
x1f = np.array([]) # np.array([MAX_VAL+0.40])#- np.random.random(10)**1.0*0.2
x1g = np.array([MAX_VAL+1.0]) - np.random.random(400)**0.33*0.5
x1h = np.array([]) # np.array([MAX_VAL+0.80])#- np.random.random(20)**1.0*0.2
x1i = np.array([]) # np.array([MAX_VAL+0.99]) - np.random.random(300)**0.33*0.33
...
raw_X = np.array(ilr.get_density_estimated_X()).flatten() # actually not raw
...
fig.set_figheight(3.000*1.111*1.111)
fig.set_figwidth(3.000*3.333)
xrange1 = (
        min((min(x0), min(x1))),
        max((max(x0), max(x1))))
...
axes[1].plot   (raw_X, raw_log_odds + offset, label='After (Step 1: adaptive kernel density estimation of $\\ln(A_f \\div B_f)$)', alpha = 0.200, marker = '^', linewidth=0.5, markersize=(16*3)**0.5) # (, linestyle='dotted')
