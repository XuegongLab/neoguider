...
def var_vcf2vep_from_bcftools(vcfrecord):
    def replace_emptystring_by_dash(ch): return (ch if (0 < len(ch)) else '-')
    chrom, pos, ref, alts = (vcfrecord.chrom, vcfrecord.start, vcfrecord.ref, vcfrecord.alts)
    alts = [a for a in alts if not a.startswith('<')]
    if not alts: return (F'{chrom}_{pos}_-/-', -1, -1, -1)
    ...
    alt2 = '/'.join([replace_emptystring_by_dash(alt[i:]) for alt in alts])
    dp = rd = ad = 0
    for sample in vcfrecord.samples:
        sample_name = sample
        this_ad = vcfrecord.samples[sample]["AD"]  # Allelic depth
        #this_dp = vcfrecord.samples[sample]["DP"]  # Total depth
        rd = max((ad, this_ad[0]))
        ad = max((ad, this_ad[1]))
        #dp = max((dp, this_dp))
    return ('_'.join([chrom2, str(pos2), ref2 + '/' + alt2]), vcfrecord.qual, int(rd), int(ad))
...
def compute_info(mhc, peptide, motifdf, zerobased_positions='all', rettype='sum'):
    ...
    if (mhc, peplen) not in motifdf:
        logging.warning(F'MHC={mhc} Len={peplen} is not found in MHC motif DB')
        return ([] if rettype=='list' else np.nan)
...
def main():
    ...
    parser.add_argument(u2d('tumor_sc_subclones_json'), default = '',
            help = 'The json file representing a dict mapping subclone ID to VCF file containing all variant')
    parser.add_argument(u2d('dna_cnv_vcf'), default = '',
            help = 'VCF file generated by calling CNVs from DNA-seq data with CNV_facets (we can extract CNVs from VCF with the `bcftools query` command)')
    ...
    cnv_file = (pysam.VariantFile(args.dna_cnv_vcf, 'r') if args.dna_cnv_vcf else [])
    tumor_purity, tumor_ploidy = None, None
    if cnv_file:
        for rec in cnv_file.header.records:
            #print((rec.key))
            #print((rec.value))
            key, value = rec.key, rec.value
            #if rec.startswith('##') and len(rec.split('=')) == 2:
            #    key, value = rec.split('=')
            if key == "purity":
                tumor_purity = (float(value) if value != 'NA' else np.nan)
            if key == "ploidy":
                tumor_ploidy = (float(value) if value != 'NA' else np.nan)
        assert tumor_purity != None, f'Th key {tumor_purity} was not found from the header of the file {args.dna_cnv_vcf}!'
        assert tumor_ploidy != None, f'Th key {tumor_ploidy} was not found from the header of the file {args.dna_cnv_vcf}!'

    subcloneID2scfiles = {}
    if args.tumor_sc_subclones_json:
        subcloneID2vcf = {}
        with open(args.tumor_sc_subclones_json) as f:
            subcloneID2vcf = json.load(f)
        for ID, vcf in subcloneID2vcf.items():
            subcloneID2scfiles[ID] = pysam.VariantFile(vcf, 'r')
    ...
    fields.append('CellularFracs_bothAlleles')
    fields.append('CellularFracs_majorAllele')
    fields.append('SubclonesWithoutEnoughCov')
    fields.append('SubclonesHavingNeoantigen')
    fields.append('SubclonesNotHavingNeoantigen')
    ...
    sc_visited_chroms = set()
    bulk_visited_chroms = set()
    prev_lineno = 0
    for lineno, line1 in enumerate(reader):
        if lineno > (prev_lineno + 1) * (5.0/3):
        ...
        if dna_snvindel and rna_snvindel and ((identity.strip().split('_')[0] in ["SNV", 'INS', 'DEL', 'INDEL', 'FSV'] or identity.strip().split('_')[0].startswith("INDEL"))):
            ...
            if len(ele) == 14: # annotation software is vep
                ...
                if subcloneID2scfiles and dna_alt_depth > 0:
                    for subcloneID, cnv_sc_file in sorted(subcloneID2scfiles.items()):
                        if not (chrom in cnv_sc_file.header.contigs):
                            if not (chrom in sc_visited_chroms):
                                logging.warning(f'The chromosome {chrom} is not found for subclone {subcloneID}')
                                sc_visited_chroms.add(chrom)
                            continue
                        for vcfrecord in cnv_sc_file.fetch(chrom, int(pos) - 6, int(pos) + 6):
                            vepvar, varqual, varRD, varAD = var_vcf2vep_from_bcftools(vcfrecord)
                            if varAD + varRD < 7:
                                undet_subclones.append(subcloneID)
                            elif varAD >= 2 and varAD >= 0.1 * (varAD+varRD) and vep_lenient_equal(vepvar, ele[0]):
                                present_subclones.append(subcloneID)
                            else:
                                absent_subclones.append(subcloneID)
                if cnv_file and dna_alt_depth > 0 and not (chrom in cnv_sc_file.header.contigs):
                    if not (chrom in bulk_visited_chroms):
                        logging.warning(f'The chromosome {chrom} is not found for the bulk CNV result file')
                        bulk_visited_chroms.add(chrom)
                elif cnv_file and dna_alt_depth > 0:
                    #for vcfrecord in cnv_file.fetch(chrom, int(pos) - 6, int(pos) + 6):
                    for vcfrecord in cnv_file.fetch(chrom):
                        #'%CHROM\t%POS\t%END\t%TCN_EM'
                        #logging.info(f'PreGoing over the CNV record {vcfrecord}')
                        #if 'END' in vcfrecord.info and vcfrecord.pos <= pos and pos <= vcfrecord.info['END']:
                        if vcfrecord.pos <= int(pos) <= vcfrecord.stop:
                            assert 'TCN_EM' in vcfrecord.info, f'The record {vcfrecord} does not have the TCN_EM field!'
                            assert 'LCN_EM' in vcfrecord.info, f'The record {vcfrecord} does not have the LCN_EM field!'
                            #assert 'CF_EM'  in vcfrecord.info, f'The record {vcfrecord} does not have the  CF_EM field!'
                            TCN_EM = vcfrecord.info['TCN_EM'] # total copy number
                            LCN_EM = vcfrecord.info['LCN_EM'] # lesser (minor) copy number
                            #CF_EM  = vcfrecord.info['CF_EM']  # clonal fraction
                            if TCN_EM != None and LCN_EM != None: # and CF_EM != None:
                                assert LCN_EM <= TCN_EM - LCN_EM, f'Record {vcfrecord} does not satisfy the criteria: LCN_EM <= TCN_EM - LCN_EM!'
                                #logging.info(f'Going over the CNV record {vcfrecord} with haploCN in {list(range(1, TCN_EM - LCN_EM, 1))}')
                                for haploCN in range(1, TCN_EM - LCN_EM + 1, 1):
                                    # CF_EM / purity = \
                                    neo_frac = varAD / float(varAD + varRD) * (tumor_purity * TCN_EM + 2*(1-tumor_purity)) / (tumor_purity * haploCN)
                                    if haploCN <= LCN_EM: neo_fracs_both.append(neo_frac)
                                    else: neo_fracs_major.append(neo_frac)
                                    #logging.info(f'  Going over the haploCN={haploCN}')
                        #else:
                        #    logging.info(f'''Skipping {vcfrecord.pos} -> {pos} -> {vcfrecord.stop}''')
            elif len(ele)==11:
        ...
        line.append('/' + '/'.join([f'{frac:.3f}' for frac in neo_fracs_both]))
        line.append('/' + '/'.join([f'{frac:.3f}' for frac in neo_fracs_major]))
        line.append('/' + '/'.join([f'{subcloneID}' for subcloneID in undet_subclones]))
        line.append('/' + '/'.join([f'{subcloneID}' for subcloneID in present_subclones]))
        line.append('/' + '/'.join([f'{subcloneID}' for subcloneID in absent_subclones]))
    ...
    for subcloneID, sc_file in sorted(subcloneID2scfiles.items()): sc_file.close()
    if cnv_file: cnv_file.close()
